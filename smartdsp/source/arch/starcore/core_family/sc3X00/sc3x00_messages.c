...
/******************************************************************************
 ...
 $Id: sc3x00_messages.c,v 1.4 2012/11/22 16:28:51 sw Exp $
 $Source: /cvsdata/SmartDSP/source/arch/starcore/core_family/sc3X00/sc3x00_messages.c,v $
 $Revision: 1.4 $
******************************************************************************/

/******************************************************************************

 @File          sc3x00_messages.c

 @Description   SC3850-specific intercore messages runtime functions.
...
inline static void interruptCorePointToPoint(uint32_t dest_core, uint16_t msg_num)
{
    osHwiSwiftDisable();
    WRITE_SYNCIO_UINT32(g_dsp_plat_map->mmu.m_gpr0,
        1 << (SOC_MAX_NUM_OF_MESSAGES * dest_core + msg_num));
    WRITE_SYNCIO_UINT32(g_dsp_plat_map->mmu.m_gpr0, 0);
    osHwiSwiftEnable();
}
                        ...
                        uint32_t        msg_data)
{
    ...
    if ((dest_core >= SOC_MAX_NUM_OF_CORES) || (dest_msg_type(msg_num, dest_core) != OS_MESSAGE_SYNC_TYPE))
    {
    ...
    WRITE_SYNCIO_UINT32(shared_messages[msg_num].messages[osGetCoreID()][dest_core].data, msg_data);
...
uint32_t osMessageGet(os_msg_handle msg_num)
{
    ...
    if (src_core > SOC_MAX_NUM_OF_CORES)
    {
#ifdef MESSAGE_ERROR_ASSERT
        OS_ASSERT;
#endif /* MESSAGE_ERROR_ASSERT */
    ...
    if (shared_messages[msg_num].messages[src_core][osGetCoreID()].guard == 0)
    {
#ifdef MESSAGE_ERROR_ASSERT
        OS_ASSERT;
#endif /*MESSAGE_ERROR_ASSERT */
...
#ifdef MESSAGE_ERROR_CHECKING
    if ((dest_core >= SOC_MAX_NUM_OF_CORES) || (dest_msg_type(msg_num, dest_core) != OS_MESSAGE_ASYNC_TYPE))
    {
#ifdef MESSAGE_ERROR_ASSERT
        OS_ASSERT;
#endif /* MESSAGE_ERROR_ASSERT */
