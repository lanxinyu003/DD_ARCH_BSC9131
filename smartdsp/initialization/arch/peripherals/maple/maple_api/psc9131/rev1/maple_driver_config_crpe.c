...
void maple_driver_config_crpe(void *input)
{
    ...
    ulbmode = maple_param->crpeulb_mode & 0x000f8083;
    ...
    if ((ulbmode&0x8000)==0) // disable ULB bit
    {
        ...
        if (1/*pstPRAM->uliMMC0P & 0x10*/) // ecc enabled
        {
            ...
            for (i=0; i<CRPE_ULB_NUM_OF_IBS; i++)
               for (j=0; j<2; j++)
               {
				   ...
				   addr = (uint32_t)(((uint8_t *)maple_param->mbus_base)+0x1c000+(i<<13)+j*0x480+0x2a0);
                   MAPLEWRITE32(addr,0);
     ...
     // DL:
     dlmode= maple_param->crpedl_mode & 0x00078017;
