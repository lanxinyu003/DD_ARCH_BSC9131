...
typedef struct
{
    ...
    /**< MAPLE mode2 set 0x3 for CRCPE Read Bus
					 ...
					 set 0x300 for EQPE Read Bus
					 set 0xc00 for EQPE Write Bus
    ...
    /**< MAPLE CONV Configuration  set 0x20 for CONV eFTPE served in high priority
                                   set 0x00 - CONVPE and external user/PUSCH EDF has the same prirority over EQPE-P.
                                   set 0x40 - CONVPE has lower prioirity than external user/PUSCH EDF when using EQPE-P.
                                   set 0x80 - CONVPE has higher priority than external user/PUSCH EDF when using EQPE-P.
                                   set 0x80000000 to Enable CONV **/
    unsigned long     conv_mode;
	...
	/**< MAPLE CRPE ULB mode
                         bit 0 is INT_MODE
                         bits 4:3 are FC_MODE
                         bit 6 is OOB mode
                         bit 7 is DLY_SPREAD
                         bit 11 is DDS
                         bits 19:16 are ISHA
                         bit 24 is double antenna enable **/
    ...
    /**< MAPLE CRPE DL mode
         ...
         bit 2 RCS
         bit 4 CDMA2K ENABLE
         bits 31:16 are antenna enables    **/
